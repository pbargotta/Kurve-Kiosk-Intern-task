...
const EditCustomerForm: React.FC<EditCustomerFormProps> = ({ customer, onCustomerUpdated, onCancel }) => {
  ...
  // useEffect to update form fields if the customer prop changes
  useEffect(() => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (name.trim() !== customer.name) customerUpdateData.name = name.trim();
    if (email.trim() !== customer.email) customerUpdateData.email = email.trim();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-60 backdrop-blur-sm">
      <div className="p-6 bg-white rounded-xl shadow-2xl w-full max-w-lg transform transition-all sm:p-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-semibold text-gray-800">Edit Customer: <span className="font-normal">{customer.name}</span></h2>
          <button 
            onClick={onCancel} 
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label htmlFor="edit-name" className="block mb-1.5 text-sm font-medium text-gray-700">Name</label>
            <input
          ...
          <div>
            <label htmlFor="edit-email" className="block mb-1.5 text-sm font-medium text-gray-700">Email</label>
            <input
          ...
          <div>
            <label htmlFor="edit-age" className="block mb-1.5 text-sm font-medium text-gray-700">Age</label>
            <input
              type="number" id="edit-age" value={age} onChange={(e) => setAge(e.target.value)}
              className="w-full px-4 py-2.5 text-gray-700 bg-gray-50 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-150 ease-in-out"
